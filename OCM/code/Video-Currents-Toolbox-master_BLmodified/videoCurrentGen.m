...
%  INPUTS
%    stack - grayscale image of timestack, size [MxN] (BL note: N--> along shore pixel number/M--> time steps)
...
%    Twin - the time length of the FFT window (in points)%BL note: (maybe)#
%    of frames that includes in the averaging process(actual calculation)
%    Tstep - time length to step the window (in points)%BL note: (maybe)#
%    of frames include in the gap between each calculation(gap between calculation)
...
%dt = abs(mean(diff(time))); %hacked by BL 
...
j = 1;
Nb = floor((size(stack,1)-(Twin-Tstep))/Tstep); %blocks%BL note:?
warning off
...
for wind = 0:(Nb-1)
    ...
    for ii = 1:length(k)
        Sv(:,ii) = interp1(f/k(ii),S(:,ii),v','linear'); %BL note 
    end
    
    % calculate S(v)
    V = sum(Sv', 'omitnan'); %BL hacked(original is the below, change nansum to sum)
    %V = nansum(Sv');
    V = colfilt(V,[1 5],'sliding',@mean);
    V = V/max(V,[],'omitnan');  %normalized spectrum %BL hacked (original below)
    %V = V/nanmax(V);  %normalized spectrum
    [maxv maxvind] = max(V,[],'omitnan'); %hacked 
    ...
    beta0 = [maxv v(maxvind) 0.25 mean(V)];% nanmedian(V) 5];
